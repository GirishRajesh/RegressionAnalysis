...
# Filter the data for high-growth sector
...
def save_summary_as_html_table(model, metric):
    ...
    # Prepare HTML output
    html_text = f"<h2>Regression Results for {metric} (High Growth Sector)</h2>\n"
    ...
    with open(f'{metric}_regression_summary_H2_High.html', 'w') as f:
        f.write(html_text)
...
# Run regression for each metric in the high-growth sector
